...
def get_db_connection_params():
    ...
    if postgres_password and (
        (
            len(postgres_password) < 15 and postgres_password.endswith("!W")
        )  # Truncated at #
        or "\\" in postgres_password  # Has escape characters
    ):
        # Password appears to be truncated or malformed; skip test with message
        print(
            "❌ Skipping: POSTGRES_PASSWORD appears truncated or malformed due to dotenv parsing issues. Please set the correct password via environment variable or .env file.",
        )
...
def test_database_connection():
    ...
    with psycopg2.connect(constructed_url) as conn, conn.cursor() as cur:
