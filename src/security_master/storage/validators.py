...
class SecurityDataValidator:
    ...
    @classmethod
    def validate_isin(cls, isin: Optional[str]) -> bool:
        ...
        num_str = ''.join(chars)
        check_sum = sum(int(digit) * (2 if i % 2 == 0 else 1) 
                       for i, digit in enumerate(reversed(num_str)))
        
        return (10 - (check_sum % 10)) % 10 == int(isin[-1])
    
    @classmethod
    def validate_wkn(cls, wkn: Optional[str]) -> bool:
        """Validate WKN format."""
        if not wkn:
            return True  # Optional field
    ...
    @classmethod
    def validate_symbol(cls, symbol: Optional[str]) -> bool:
