...
class PPAccount(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    uuid: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), unique=True, nullable=False, default=uuid4)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    currency_code: Mapped[str] = mapped_column(String(3), nullable=False, default="USD")
    is_retired: Mapped[bool] = mapped_column(Boolean, default=False)
    
    # Account attributes (PP XML properties)
    attributes: Mapped[Optional[str]] = mapped_column(Text)  # JSON storage for PP attributes
    ...
    # Relationships
    transactions: Mapped[list["PPAccountTransaction"]] = relationship("PPAccountTransaction", back_populates="account")
    portfolios: Mapped[list["PPPortfolio"]] = relationship("PPPortfolio", back_populates="reference_account")
...
class PPPortfolio(Base):
    ...
    reference_account_id: Mapped[Optional[int]] = mapped_column(ForeignKey("pp_accounts.id"))
    reference_account: Mapped[Optional["PPAccount"]] = relationship("PPAccount", back_populates="portfolios")
    ...
    # Relationships
    transactions: Mapped[list["PPPortfolioTransaction"]] = relationship("PPPortfolioTransaction", back_populates="portfolio")
...
class PPAccountTransaction(Base):
    ...
    account_id: Mapped[int] = mapped_column(ForeignKey("pp_accounts.id"), nullable=False)
    account: Mapped["PPAccount"] = relationship("PPAccount", back_populates="transactions")
    ...
    security_id: Mapped[Optional[int]] = mapped_column(ForeignKey("securities_master.id"))
    ...
    cross_entry_type: Mapped[Optional[str]] = mapped_column(String(50))  # "buysell", "dividend", etc.
...
class PPPortfolioTransaction(Base):
    ...
    portfolio_id: Mapped[int] = mapped_column(ForeignKey("pp_portfolios.id"), nullable=False)
    portfolio: Mapped["PPPortfolio"] = relationship("PPPortfolio", back_populates="transactions")
    ...
    security_id: Mapped[int] = mapped_column(ForeignKey("securities_master.id"), nullable=False)
    ...
    linked_account_transaction_id: Mapped[Optional[int]] = mapped_column(ForeignKey("pp_account_transactions.id"))
...
class PPTransactionUnit(Base):
    ...
    transaction_type: Mapped[str] = mapped_column(String(20), nullable=False)  # 'ACCOUNT' or 'PORTFOLIO'
...
class PPSecurityPrice(Base):
    ...
    price_value: Mapped[int] = mapped_column(Integer, nullable=False)  # PP stores as integer (multiply by 100000000)
    
    # Data source tracking
    price_source: Mapped[str] = mapped_column(String(50), default="manual")  # manual, yahoo, pp, etc.
...
class PPSetting(Base):
    ...
    setting_category: Mapped[str] = mapped_column(String(50), nullable=False)  # 'properties', 'bookmarks', etc.
    ...
    __table_args__ = (
        UniqueConstraint('setting_category', 'setting_key', name='uq_setting_category_key'),
    )
