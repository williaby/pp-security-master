...
class PPXMLExportService:
    ...
    def generate_complete_backup(self, config_name: str = "default") -> str:
        ...
        config = self.session.query(PPClientConfig).filter_by(
    ...
    def _add_securities_section(self, parent: ET.Element) -> None:
        ...
        for security in securities:
            ...
            ET.SubElement(security_elem, "uuid").text = str(security.id)  # Use database ID as UUID for now
            ET.SubElement(security_elem, "name").text = security.name
            ET.SubElement(security_elem, "currencyCode").text = security.currency or "USD"
    ...
    def _add_price_history(self, security_elem: ET.Element, security_id: int) -> None:
        ...
        prices = (self.session.query(PPSecurityPrice)
    ...
    def _add_accounts_section(self, parent: ET.Element) -> None:
        ...
        for account in accounts:
            ...
            ET.SubElement(account_elem, "isRetired").text = str(account.is_retired).lower()
            ...
            ET.SubElement(account_elem, "updatedAt").text = datetime.utcnow().isoformat() + "Z"
    
    def _add_account_transactions(self, account_elem: ET.Element, account_id: int) -> None:
        """Add all transactions for an account."""
        transactions = (self.session.query(PPAccountTransaction)
        ...
        if transactions:
            ...
            for transaction in transactions:
                ...
                ET.SubElement(trans_elem, "date").text = transaction.transaction_date.strftime("%Y-%m-%dT00:00")
                ET.SubElement(trans_elem, "currencyCode").text = transaction.currency_code
                ET.SubElement(trans_elem, "amount").text = str(int(transaction.amount * 100))  # PP uses cents
                
                # Security reference if present
                if transaction.security_id:
                    security_ref = ET.SubElement(trans_elem, "security")
                    security_ref.set("reference", f"../../../../../securities/security[{transaction.security_id}]")
                
                # Shares (usually 0 for account transactions)
                ET.SubElement(trans_elem, "shares").text = str(int(transaction.shares * 100000000))  # PP format
    ...
    def _add_transaction_units(self, transaction_elem: ET.Element, transaction_id: int, transaction_type: str) -> None:
        """Add fee and tax units for a transaction."""
        units = (self.session.query(PPTransactionUnit)
    ...
    def _add_portfolios_section(self, parent: ET.Element) -> None:
        ...
        for portfolio in portfolios:
            ...
            ET.SubElement(portfolio_elem, "isRetired").text = str(portfolio.is_retired).lower()
            
            # Reference account
            if portfolio.reference_account:
                ref_account_elem = ET.SubElement(portfolio_elem, "referenceAccount")
                ET.SubElement(ref_account_elem, "uuid").text = str(portfolio.reference_account.uuid)
                ET.SubElement(ref_account_elem, "name").text = portfolio.reference_account.name
                ET.SubElement(ref_account_elem, "currencyCode").text = portfolio.reference_account.currency_code
                ET.SubElement(ref_account_elem, "isRetired").text = str(portfolio.reference_account.is_retired).lower()
                ...
                # Updated timestamp
                ET.SubElement(ref_account_elem, "updatedAt").text = datetime.utcnow().isoformat() + "Z"
            
            # Add portfolio transactions
            self._add_portfolio_transactions(portfolio_elem, portfolio.id)
    
    def _add_portfolio_transactions(self, portfolio_elem: ET.Element, portfolio_id: int) -> None:
        """Add all transactions for a portfolio."""
        transactions = (self.session.query(PPPortfolioTransaction)
        ...
        if transactions:
            ...
            for transaction in transactions:
                ...
                if transaction.linked_account_transaction:
                    ...
                    portfolio_trans_ref = ET.SubElement(cross_entry, "portfolioTransaction")
                    ...
                    ET.SubElement(account_trans, "date").text = linked_trans.transaction_date.strftime("%Y-%m-%dT00:00")
                    ET.SubElement(account_trans, "currencyCode").text = linked_trans.currency_code
                    ET.SubElement(account_trans, "amount").text = str(int(linked_trans.amount * 100))
                    
                    if linked_trans.security_id:
                        security_ref = ET.SubElement(account_trans, "security")
                        security_ref.set("reference", f"../../../../../../../../../../../securities/security[{linked_trans.security_id}]")
                    ...
                    ET.SubElement(account_trans, "shares").text = str(int(linked_trans.shares * 100000000))
                    ET.SubElement(account_trans, "type").text = linked_trans.transaction_type
    ...
    def _add_properties_section(self, parent: ET.Element) -> None:
        ...
        properties = (self.session.query(PPSetting)
                     .filter_by(setting_category="properties")
                     .all())
