...
class ServiceMonitor:
    ...
    def __init__(self, config: Optional[ServiceResilienceConfig] = None):
    ...
    def add_alert_handler(self, handler: Callable[[str, ServiceMetrics], Awaitable[None]]):
    ...
    async def execute_with_resilience(self,
        ...
        if not circuit_breaker.can_execute():
            self.logger.warning(f"Circuit breaker OPEN for {service_name}, using fallback")
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if attempt == self.config.max_retries:
                    self.logger.error(f"All attempts failed for {service_name}, using fallback")
                    return await self._execute_fallback(service_name, fallback_value)
    ...
    async def _run_health_checks(self):
        ...
        for service_name, health_check in self.health_checks.items():
            task = asyncio.create_task(self._run_single_health_check(service_name, health_check))
            tasks.append(task)
        
        if tasks:
            await asyncio.gather(*tasks, return_exceptions=True)
    
    async def _run_single_health_check(self, 
                                     service_name: str,
                                     health_check: Callable[[], Awaitable[HealthCheckResult]]):
        ...
        try:
            ...
            if result.status == ServiceStatus.HEALTHY:
                ...
                self._record_failure(service_name, result.error or "Health check failed")
    ...
    async def _check_alerts(self):
        ...
        for service_name, metrics in self.services.items():
            if (metrics.failure_rate >= self.config.alert_threshold_failure_rate or
                metrics.current_status == ServiceStatus.UNHEALTHY):
...
async def openfigi_health_check() -> HealthCheckResult:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get('https://api.openfigi.com/v3/search',
                                 params={'query': 'test'},
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                json={'text': f"🚨 Service {service_name} is {metrics.current_status.value}"},
